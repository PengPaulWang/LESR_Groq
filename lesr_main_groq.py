...
import pynvml
import shutil 
from groq import Groq  # Assuming GroqAPI is the library for Groq's API
import pysnooper

#  python lesr_main_groq.py --env HalfCheetah-v4  --intrinsic_w 0.02 --sample_count 6 --iteration 3 --model llama3-8b-8192 --cuda 0 --temperature 0.0 --max_timesteps 800000 --max_evaluate_timesteps 1000000 --groq_key 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
...
# @pysnooper.snoop()
def call_groq_api(model, messages, temperature):
    try:
...
{detail_content}
You should design a task-related state representation based on the source {total_dim} dim to better for reinforcement training, using the detailed information mentioned above to do some calculations, and feel free to do complex calculations, and then concat them to the source state.
...
3. to better design the intrinsic_reward, we recommend you use some source dim in the updated_s, which is between updated_s[0] and updated_s[{total_dim - 1}] 
...
#             print(f"Window index {idx} in use, trying next index...")
#             continue
...
#             w = session.new_window(attach=False, window_index=idx)
#             time.sleep(2)
#             print(f"Created new window with index {idx}")
#         except libtmux.exc.LibTmuxException as e:
#             print(f"Window index {idx} in use, trying next index...")
#             w = session.find_where({'window_index': str(idx)})
#             if not w:
#                 print(f"Failed to find window with index {idx}, continuing...")
#                 continue
#         try:
#             pm = w.list_panes()[0]
#             lastLine = pm.cmd('capture-pane', '-p').stdout[-1]
#             if args.user_name + '@' in lastLine and lastLine.endswith("$"):
#                 window_can_use = True
#                 print(f"Window {idx} is available")
#             else:
#                 print(f"Window {idx} is occupied")
#         except Exception as e:
#             print(f"Exception occurred while checking window {idx}: {e}")
#             continue

#     ### put the command into window to run ###
#     print("=======================================")
#     print(f"Current Command: {command} Current Window: {idx}")
#     print("=======================================")
...
def find_window_and_execute_command(command):
    window_can_use = False
    idx = 5  # Start check from window 5
    max_windows = 60
    ...
    while not window_can_use:
        ...
        try:
            ...
            print(f"Window index {idx} in use, trying next index...")
            w = next((win for win in session.windows if win.index == str(idx)), None)
            if not w:
                print(f"Failed to find window with index {idx}, continuing...")
                continue
        try:
            pm = w.attached_pane
            last_line = pm.capture_pane(start=-1, end=-1)[-1]
            if args.user_name + '@' in last_line and last_line.endswith("$"):
                window_can_use = True
                print(f"Window {idx} is available")
            else:
                print(f"Window {idx} is occupied")
...
def get_cot_prompt(every_code, every_score, max_id, every_factor, every_dim, state_dim):
    ...
    for ii in range(len(every_code)):
        ...
        try:
            s_feedback += f'In this State Revise Code {ii + 1}, the source state dim is from s[0] to s[{state_dim - 1}], the Lipschitz constant between them and the reward are(Note: The Lipschitz constant is always greater than or equal to 0, and a lower Lipschitz constant implies better smoothness.):\n'
...
def get_next_iteration_prompt(all_it_func_results, all_it_cot_suggestions):
    ...
    for ii in range(len(all_it_func_results)):
        ...
        former_history += all_it_func_results[ii]
...
# def run_iterations(args): 
    ...
    parser.add_argument("--model", default="llama3-8b-8192", type=str)                                          # which llm should use
    parser.add_argument("--groq_key", default="", type=str)                                                     # groq key
    ...
    assert groq_key != '', 'You should pass a Groq Key to get access to the model.'
    ...
    ### init tmux server ###
    server = libtmux.Server()
    session = next((s for s in server.sessions if s.name == args.session_name), None)
    if not session:
        session = server.new_session(session_name=args.session_name)
    assert session
    ...
    for it in range(args.iteration):
        ...
        while valid_sample_count != args.sample_count:
            ...
            try:
                ...
                # completion = call_groq_api(prompt, args.model, temperature)
                completion = call_groq_api(**params)
                sid_code = completion
                ...
                try:
                    ...
                    print("No 'return' statement found in the code snippet.")
                    continue
        ...
        while True:
            ...
            for train in range(args.sample_count):
                # print(f'Current Iteration:{it} Current Train:{train}')
                for seed_try in range(args.train_seed_count):
                    # print(f'Current Iteration:{it} Current Train:{train} Current Seed:{seed_try}')
                    cur_sid_result_path = f'{sid_result_dir}/it{it}_train{train}_s{seed_try}.npy'
                    cur_corr_result_path = f'{sid_result_dir}/it{it}_train{train}_corr_s{seed_try}.npy'
                    # print(f'Current Iteration:{it} Current Train:{train} Current Seed:{seed_try} Current Path:{cur_sid_result_path}')
                    if os.path.exists(cur_corr_result_path):
                        print(f'Current Iteration:{it} Current Train:{train} Current Seed:{seed_try} Current Path:{cur_sid_result_path} Exists')
        ...
        ### Get analysis first, then get the final answer
        """
        ...
        while True:
            try:
                ...
                # completion = call_groq_api(prompt, args.model, temperature)
                completion = call_groq_api(**params)
                cur_it_cot_suggestions = completion
    ...
    while True:
        ...
        for train in range(args.evaluate_count):
            ...
            if os.path.exists(cur_sid_result_path):
                evaluate_results.append(np.load(cur_sid_result_path))
                finish_count += 1
